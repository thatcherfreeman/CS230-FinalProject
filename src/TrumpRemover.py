...
def process_through_model(path_to_input_wav: str, path_to_output: str, model: nn.Module, alpha: float = 0.85) -> AudioData:
    ...
    output_segments = apply_model(input_freq_domain, model, input_segments[0], alpha)
...
def apply_model(input: np.ndarray, model: nn.Module, example_stft: StftData, alpha: float) -> List[AudioData]:
